...
func (s *StateStore) UpsertHostVolumes(index uint64, volumes []*structs.HostVolume) error {
	...
	for _, v := range volumes {
		...
		if obj != nil {
			// Only some properties of a volume can be updated in place.
			//
			// TODO(1.10.0): we need to check this in the RPC handler before
			// sending the ClientRPC, so figure out if we really need to do this
			// here.
			old := obj.(*structs.HostVolume)
			if old.PluginID != v.PluginID {
				return fmt.Errorf("host volume plugin cannot be updated: %s", v.ID)
			}
